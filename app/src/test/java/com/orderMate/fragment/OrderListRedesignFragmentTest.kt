...
/**
 * Unit tests for OrderListRedesignFragment's REST backfill / infinite-scroll logic (#138/#150).
 *
 ...
 * so, following this codebase's existing convention (see WidgetManagerTest,
 * OrderFilterUtilsTest), these tests mirror the pure merge/pagination logic locally with
 * lightweight stand-ins rather than the real Order/Fragment classes.
 */
class OrderListRedesignFragmentTest {
    ...
    // orders Clover's REST API actually returned, regardless of how many were new to us - this
    // is what keeps the eager backfill paging forward instead of re-fetching the same page.
    ...
    fun `offset advances by the full page size returned, not by new-order count`() {
        ...
        // advance by 50, or the next page would re-fetch orders we've already seen.
    ...
    // Mirrors loadOrders()'s reload-merge step: a fresh read of Clover's local,
    // retention-windowed cache can omit orders that were previously pulled in via the REST
    // backfill (e.g. after the merchant scrolled down, then hit Sync). Those backfilled orders
    // must be re-merged back in rather than dropped, or they "disappear" again on every reload
    // even though the scroll-backfill itself works.
